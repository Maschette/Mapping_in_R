---
title: "Mapping in R"
author: "Dale Maschette, Ben Raymond & Michael Sumner"
date: "02/10/2019"
...
knitr::include_graphics("./Maps_are_hard.png", dpi = 75)
```
...
```{r ggplot2}
ggplot(world, aes(x=lon, y=lat)) +
  geom_polygon(aes(group = group, fill=id), col="grey") + 
  coord_quickmap()+scale_fill_discrete(guide=FALSE)
```
...
ggplot(world, aes(x=lon, y=lat)) +
  geom_polygon(aes(group = group, fill=id), col="grey") + 
  coord_map(projection = "stereographic", orientation = c(-90, 0, 0))+scale_fill_discrete(guide=FALSE)
```
...
ggplot(world, aes(x=lon, y=lat)) +
  geom_polygon(aes(group = group, fill=id), col="grey") + 
  coord_map(projection = "ortho", orientation = c(-90, 0, 0))+scale_fill_discrete(guide=FALSE)
```

Adding some data? We probably need some first so we will make some fake data quickly
```{r ggplot3}
dummy<-data.frame(lon=rnorm(100, mean=0, sd=60),lat= rnorm(100, mean=0, sd=20))
```
...
ggplot(world, aes(x=lon, y=lat)) +
  geom_polygon(aes(group = group, fill=id), col="grey") + 
  coord_quickmap()+scale_fill_discrete(guide=FALSE)+
  geom_point(data=dummy, aes(x=lon, y=lat), col="black")
```
...
```{r sobin2}
SOmap()
SOplot(col=viridis::viridis(100),
  SOmap:::SObin(x=points$lon, y=points$lat, dim=c(100,100)))
```
...
```{r citation, fig.cap="Fig. inf: shiny Southern Ocean map made with SOmap (Maschette et al., 2019). "}
SOmap()
```
